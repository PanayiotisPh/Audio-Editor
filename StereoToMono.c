...
void stereoToMono(char *input, char *output){
    ...
    for (i=0;i<wave->dataChunk->subchunk2Size; i=i+ (wave->fmtChunk->bitsPerSample)/4){
        memcpy(&monoWave->dataChunk->data[i/2], &wave->dataChunk->data[i], wave->fmtChunk->bitsPerSample/8);
        insertCounter = insertCounter+wave->fmtChunk->bitsPerSample/8;
    }
    printf("InsertCounter: %d\ti: %d\n", insertCounter,i);
...
void initializeMonoWave(WAVE *monoWave, WAVE *wave){
    ...
    monoWave->fmtChunk->byteRate = wave->fmtChunk->byteRate/2;
    ...
    monoWave->dataChunk->subchunk2Size = (OldNumSample) * (wave->fmtChunk->bitsPerSample / 8);    //mono subchunk2size
